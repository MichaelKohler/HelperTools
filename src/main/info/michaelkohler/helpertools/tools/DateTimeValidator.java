...
/**
 * A class that provides convenient methods to validate dates and times values based
 * on the Gregorian calendar. @see {@link GregorianCalendar}.
 *
 * @author isim
 * @version 0.0.1
 *
...
public final class DateTimeValidator {
    ...
    /**
     * Check to see if the specified year, month and day can be formulated into a
     * legitimate date in the Gregorian calendar.
     *
     * @param year Year to be checked.
     * @param month Month to be checked.
     * @param day Day to be checked.
     * @throws {@link IllegalArgumentException} if the argument cannot be
     *         formulated into a legitimate date in the Gregorian calendar.
     */
    public static void validateDateArguments(int year, int month, int day) {
        ...
        if (!monthWithinRange(month))
            throw new IllegalArgumentException("Month argument falls outside of the "
                                               + "supported range of 1 - 12");
        if (!dayWithinRange(day))
            throw new IllegalArgumentException("Day argument falls outside of the "
                                               + "supported range of 1 - 31");
        ...
        } else if (isApril(month) || isJune(month) || isSeptember(month)
                   || isNovember(month)) {
            if (day >= 31) {
                throw new IllegalArgumentException();
            }
    ...
    /**
     * Check to see if the specified year, month, day, hour, minute and second
     * can be formulated into a legitimate date-time in the gregorian calendar.
     *
     * @param year Year to be checked.
     * @param month Month to be checked.
     * @param day Day to be checked.
     ...
     * @throws {@link IllegalArgumentException} if the specified year, month,
     *         day, hour, minute and second cannot be formulated into a legitimate date
    ...
    private static void validateTimeArguments(int hour, int minute, int second) {
        if (!hourWithinRange(hour))
            throw new IllegalArgumentException("Hour argument falls outside of the"
                                               + "supported range of 0 - 23");
        if (!minuteWithinRange(minute))
            throw new IllegalArgumentException("Minute argument falls outside of the"
                                               + "supported range of 0 - 59");
        if (!secondWithinRange(second))
            throw new IllegalArgumentException("Second argument falls outside of the"
                                               + "supported range of 0 - 59");
    }
